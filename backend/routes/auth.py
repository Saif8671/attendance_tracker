...
def signup():
    ...
    form = {"role": role, "name": name, "username": username, "email": email, "phone": phone, "class_name": class_name, "rollno": rollno}
...
def reset_my_password():
    ...
    new_pw = request.form.get("new_password", "").strip()
    db = get_db()
    user = db.execute("SELECT * FROM users WHERE id=%s AND password=%s", (session["user_id"], hash_pw(old_pw))).fetchone()
    if not user:
        role = session["role"]
